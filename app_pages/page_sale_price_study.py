...
def page_sale_price_study_body():
    ...
    vars_to_study = [
        '1stFlrSF', 'GarageArea', 'GarageFinish', 'GarageYrBlt',
        'GrLivArea', 'KitchenQual', 'OverallQual', 'TotalBsmtSF',
    ...
    if st.checkbox("Inspect Customer Base"):
        st.write(
            f"* The dataset has {df_saleprice.shape[0]} rows and "
            f"{df_saleprice.shape[1]} columns, find below the first 10 rows.")
    ...
    st.write(
        ...
        f"understand how the variables are correlated to House "
        f"sale prices (SalePrice). \n"
        f"The most correlated variable (studied variables) "
        f"are: **{vars_to_study}**"
    )

    # Text based on "02 - Churned Customer Study" notebook
    # "Conclusions and Next steps" section
    st.info(
        f"The correlations and plots interpretation converge.\n"
        f"* Larger floor space (GrLivArea, 1stFlrSF, GarageArea  "
        f"and TotalBsmtSF) is associated with a higher SalePrice\n"
        f"* Combined TotalLivArea, which includes above and below "
        f"ground correlates stronger to a higher SalePrice.\n"
        f"* GrLivArea has the highest individual correlation to a higher "
        f"SalePrice.\n"
        f"* Newer built properties (YearBuilt) are higher in SalePrice.\n"
        f"* Recently built garages and refurbishments (GarageYrBlt, "
        f"YearRemodAdd) are associated with a higher SalePrice\n"
        f"* Higher overall quality houses (OveralQual) are associated "
        f"with a higher SalePrice\n"
        f"* Higher quality kitchens and garages also contribute "
        f"(less strong correlation) to a higher SalePrice "
        f"(GarageFinish and KitchenQual)\n"
    )

    # Code copied from "02 - SalePrice Study" notebook
    # "EDA on selected variables" section
    ...
    if st.checkbox("Correlation of variables to SalePrice"):
        ...
        st.write(
            f"The following table displays both the Pearson and Spearman "
            f"correlation scores of the studied variables and SalePrice"
            )
    ...
    if st.checkbox("Variable per SalePrice"):
        st.write(
            f"The following figures display each studied "
            f"variable plotted against the SalePrice"
            )
    ...
    if st.checkbox("Correlation of variable, after addition, to SalePrice"):
        st.write(
            f"The following table displays both the Pearson and "
            f"Spearman correlation scores of the studied variables "
            f"and new 'TotalLivArea' variable "
            f"against SalePrice. \n\n"
            f"Note TotalLivArea is the sum of 'GrLivArea' and 'TotalBsmtSF'"
            )
        corr_spearman_eda_new = df_eda.corr(
            method='spearman')['SalePrice'].sort_values(
                key=abs, ascending=False)[1:].head(11)
    ...
    if st.checkbox("TotalLivArea per SalePrice"):
        st.write(
            f"The following figures display the new variable "
            f"'TotalLivArea' plotted against the SalePrice"
            )
...
def plot_regression(df, col, target_var, alpha_scatter=0.5):
    ...
    sns.regplot(
        data=df, x=col, y=target_var,
        scatter_kws={'alpha': alpha_scatter}, line_kws={'color': 'red'})
...
def sale_price_per_variable(df):
    ...
    vars_to_study_numerical = [
        '1stFlrSF', 'GarageArea', 'GarageYrBlt',
